...
export class PassportPage {
  ...
  async ionViewDidEnter() {
    ...
    try {
      const search = new URLSearchParams(this.route.snapshot.fragment);
      ...
      this.isLoading = true;
      const { passport } = await this.apiService
        .getSharedPassportViaToken(token)
        .toPromise();
      // const { passport } = await this.apiService
      //   .validateSharedPassport(shared.passportId, shared.id)
      //   .toPromise();
